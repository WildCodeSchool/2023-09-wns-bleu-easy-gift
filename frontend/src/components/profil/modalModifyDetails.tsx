...
export default function ModalModifyDetails({ isOpen, handleClose, user }: ModalModifyDetailsProps) {
  ...
  const onConfirm = (e: React.FormEvent) => {
    e.preventDefault();
    try {
      updateUserMutation({ variables: { data: { email: email, pseudo: pseudo } } });
      handleClose();
  ...
  const [updateUserMutation, { loading: updating, error: updateError }] = useUpdateUserMutation();
  ...
  return (
    <div className="absolute inset-0 flex justify-center items-center">
      <div className="fixed inset-0 bg-black/50 z-50">
        <div ref={modalContentRef} className="bg-white w-9/12 md:w-7/12 lg:w-4/12 2xl:w-3/12 xl:p-5" style={modalStyles}>
          <div className="p-3">
            <div className="flex justify-between ">
              <p className="mb-9 text-lg text-left md:mb-10 md:text-xl text-primaryBlue">Modifier mes informations</p>
              <svg
            ...
            <div className="flex justify-center">
              <form className="gap-2" onSubmit={onConfirm}>
                ...
                <Input id="email" type="email" name="email" label="Modifier votre email" value={email} onChange={(e) => setEmail(e.target.value)} />
