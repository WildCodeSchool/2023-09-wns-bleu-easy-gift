...
const MenuDiscussions = ({
    ...
    return (
        <nav
            className={`bg-slate-200 w-full h-full pt-5 pb-6 flex flex-col justify-start md:pb-6 md:w-5/12 md:max-w-screen-sm md:h-auto md:justify-between md:overflow-y-auto md:shadow-[-11px_6px_21px_3px_theme(colors.slate.500)] lg:justify-start transition-all duration-1000 ease-in-out ${isMenuHidden ? 'w-0 max-w-0 md:w-0 md:max-w-0 overflow-hidden -translate-x-full opacity-25' : 'w-full md:w-5/12 translate-x-0 opacity-100'}
    `}
        >
