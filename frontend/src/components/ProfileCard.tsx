      ...
      <div className="flex m-3 lg:min-w-72 items-center gap-3 p-2 ">
        ...
        <div>
          <div className="flex justify-between">
            <div className="font-bold">{userToGroup.user.pseudo}</div>
          </div>
          <div className="text-sm text-gray-500">{userToGroup.user.email}</div>
        </div>
      </div>
      <div className="mt-2 mr-3">{userToGroup.is_admin && <Badge variant="outline">Admin</Badge>}</div>
    </div>
  );
}
