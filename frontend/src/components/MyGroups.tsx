...
function MyGroups({ isConnected, groups, groupsLoading, groupsError }: MyGroupsProps) {
        ...
        {groups.map((group) => (
          <Card className="w-80 shadow-lg hover:scale-105 shadow-slate-300 transition-transform duration-300 ease-in-out flex-grow md:max-w-[318px] md:shadow-slate-400">
            <CardHeader className="bg-white shadow-sm h-350 rounded-t-lg">
              <img src={group.avatar.url} alt={group.avatar.name} className="w-full h-full object-cover rounded-t-lg mb-4" />
              <CardTitle className="font-bold text-xl mb-2">{group.name}</CardTitle>
            </CardHeader>
            <div className="flex justify-end align-middle m-5" role="button" id="Accéder au groupe" onClick={() => router.push("/")}>
              <svg
